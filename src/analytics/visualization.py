...
def _fig_path(run_id: str, name: str) -> str:
    os.makedirs(SETTINGS.FIGURES_DIR, exist_ok=True)
    return os.path.join(SETTINGS.FIGURES_DIR, f"{run_id}_{name}.png")
...
def plot_hist(df: pd.DataFrame, column: str, run_id: str, bins: int = 30) -> str:
    ...
    ax.set_ylabel("Count")
    out = _fig_path(run_id, f"hist_{column}")
...
def plot_box(df: pd.DataFrame, column: str, run_id: str) -> str:
    ...
    out = _fig_path(run_id, f"box_{column}")
    fig.savefig(out, bbox_inches="tight")
    plt.close(fig)
    return out
...
def plot_bar(df: pd.DataFrame, cat: str, val: Optional[str], run_id: str, topn: int = 20) -> str:
    ...
    ax.set_title(title)
    ax.set_xlabel(cat)
    out = _fig_path(run_id, f"bar_{cat}{'_' + val if val else ''}")
    fig.savefig(out, bbox_inches="tight")
    plt.close(fig)
    return out
...
def plot_scatter(df: pd.DataFrame, x: str, y: str, run_id: str) -> str:
    ...
    ax.set_xlabel(x)
    ax.set_ylabel(y)
    out = _fig_path(run_id, f"scatter_{x}_{y}")
    fig.savefig(out, bbox_inches="tight")
    plt.close(fig)
    return out
...
def plot_datetime_counts(df: pd.DataFrame, datetime_col: str, run_id: str, freq: str = "M") -> str:
    ...
    if ser.empty:
        ...
        out = _fig_path(run_id, f"dt_counts_{datetime_col}_{freq}")
        fig.savefig(out, bbox_inches="tight")
        plt.close(fig)
        return out
...
def plot_time_of_day_hist(df: pd.DataFrame, datetime_col: str, run_id: str) -> str:
    ...
    if ser.empty:
        ...
        out = _fig_path(run_id, f"dt_hour_{datetime_col}")
        fig.savefig(out, bbox_inches="tight")
        plt.close(fig)
        return out
...
def plot_month_box(df: pd.DataFrame, datetime_col: str, value_col: str, run_id: str) -> str:
    ...
    if not valid.any():
        ...
        out = _fig_path(run_id, f"dt_month_box_{value_col}_by_{datetime_col}")
        fig.savefig(out, bbox_inches="tight")
        plt.close(fig)
        return out
