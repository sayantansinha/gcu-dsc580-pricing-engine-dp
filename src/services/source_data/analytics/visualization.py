...
from src.services.data_io import save_figure
...
def plot_hist(df: pd.DataFrame, column: str, run_id: str, bins: int = 30) -> str:
    ...
    ax.set_ylabel("Count")
    save_figure(fig, _fig_name(run_id, f"hist_{column}"))
    plt.close(fig)
    return out
...
def plot_box(df: pd.DataFrame, column: str, run_id: str) -> str:
    ...
    save_figure(fig, _fig_name(run_id, f"box_{column}"))
    plt.close(fig)
    return out
...
def plot_bar(df: pd.DataFrame, cat: str, val: Optional[str], run_id: str, topn: int = 20) -> str:
    ...
    ax.set_title(title)
    ax.set_xlabel(cat)
    save_figure(fig, _fig_name(run_id, f"bar_{cat}{'_' + val if val else ''}"))
    plt.close(fig)
    return out
...
def plot_scatter(df: pd.DataFrame, x: str, y: str, run_id: str) -> str:
    ...
    ax.set_xlabel(x)
    ax.set_ylabel(y)
    save_figure(fig, _fig_name(run_id, f"scatter_{x}_{y}"))
    plt.close(fig)
    return out
...
def plot_datetime_counts(df: pd.DataFrame, datetime_col: str, run_id: str, freq: str = "M") -> str:
    ...
    if ser.empty:
        ...
        save_figure(fig, _fig_name(run_id, f"dt_counts_{datetime_col}_{freq}"))
        plt.close(fig)
        return out
...
def plot_time_of_day_hist(df: pd.DataFrame, datetime_col: str, run_id: str) -> str:
    ...
    if ser.empty:
        ...
        save_figure(fig, _fig_name(run_id, f"dt_hour_{datetime_col}"))
        plt.close(fig)
        return out
...
def plot_month_box(df: pd.DataFrame, datetime_col: str, value_col: str, run_id: str) -> str:
    ...
    if not valid.any():
        ...
        save_figure(fig, _fig_name(run_id, f"dt_month_box_{value_col}_by_{datetime_col}"))
        plt.close(fig)
        return out
